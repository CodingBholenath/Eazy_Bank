...
import
        com.eazybytes.cards.constants.CardsConstants;
...
@Tag(
        ...
        description = "CRUD REST API for Cards in EazyBank to CREATE,UPDATE,FETCH AND DELETE card details"
)
@RestController
...
private Logger logger= LoggerFactory.getLogger(CardsController.class);
 ...
 @Autowired
    ...
    @Operation(
            summary = "Create a new card Rest api",
    ...
    public ResponseEntity<ResponseDto> createCard(@Valid @RequestParam
                                                  @Pattern(regexp = "[0-9]{10}", message = "The length of number should be 10")
                                                  String mobileNumber) {
    ...
    public ResponseEntity<ResponseDto> updateCardDetails(@Valid @RequestBody CardsDto cardsDto) {
        ...
        if (isUpdated) {
            ...
            return ResponseEntity.status(
                    HttpStatus.EXPECTATION_FAILED).body(new ResponseDto(CardsConstants.STATUS_417, CardsConstants.MESSAGE_417_UPDATE)
            );
        }
    }
